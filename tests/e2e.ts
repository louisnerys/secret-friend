...
// Insert usuarios profiles
section("PASSO 1 — Tabela usuarios (INSERT + RLS)");

const insertU1 = await rest("POST", "/rest/v1/usuarios", 
  { id: user1Id, email: email1, nome: "User Test 1" },
  { Authorization: `Bearer ${user1Token}` }
);
if (insertU1.status === 201 || insertU1.status === 200) {
  ok("User 1 inserido na tabela usuarios");
} else {
  fail("INSERT usuarios user1", `Status ${insertU1.status}: ${JSON.stringify(insertU1.data)}`);
}

const insertU2 = await rest("POST", "/rest/v1/usuarios",
  { id: user2Id, email: email2, nome: "User Test 2" },
  { Authorization: `Bearer ${user2Token}` }
);
if (insertU2.status === 201 || insertU2.status === 200) {
  ok("User 2 inserido na tabela usuarios");
} else {
  fail("INSERT usuarios user2", `Status ${insertU2.status}: ${JSON.stringify(insertU2.data)}`);
}

const insertU3 = await rest("POST", "/rest/v1/usuarios",
  { id: user3Id, email: email3, nome: "User Test 3" },
  { Authorization: `Bearer ${user3Token}` }
);
if (insertU3.status === 201 || insertU3.status === 200) {
  ok("User 3 inserido na tabela usuarios");
} else {
  fail("INSERT usuarios user3", `Status ${insertU3.status}: ${JSON.stringify(insertU3.data)}`);
}

// Test SELECT on usuarios (policy: anyone can view)
const selectUsers = await rest("GET", "/rest/v1/usuarios?select=id,email,nome",
...
if (selectUsers.status === 200 && Array.isArray(selectUsers.data) && (selectUsers.data as unknown[]).length >= 2) {
  ok("SELECT usuarios retorna todos os usuários", `${(selectUsers.data as unknown[]).length} registros`);
} else {
  fail("SELECT usuarios", `Status ${selectUsers.status}: ${JSON.stringify(selectUsers.data)}`);
}

// Create an event
section("PASSO 1 — Tabela eventos");

const createEvento = await rest("POST", "/rest/v1/eventos",
  { criador_id: user1Id, nome: `E2E Evento ${ts}`, descricao: "Teste E2E" },
  { 
    Authorization: `Bearer ${user1Token}`,
    Prefer: "return=representation"
  }
);
if (createEvento.status === 201 || createEvento.status === 200) {
  const eventos = createEvento.data as { id: string }[];
  eventoId = eventos[0]?.id ?? "";
  ok("Evento criado pelo User 1", `id=${eventoId.slice(0, 8)}...`);
} else {
  fail("INSERT eventos", `Status ${createEvento.status}: ${JSON.stringify(createEvento.data)}`);
}

// User 2 should NOT be able to see the event (not a participant yet)
const u2SeeEvent = await rest("GET", `/rest/v1/eventos?id=eq.${eventoId}&select=id,nome`,
...
if (u2SeeEvent.status === 200 && Array.isArray(u2SeeEvent.data) && (u2SeeEvent.data as unknown[]).length === 0) {
  ok("RLS eventos: User 2 NÃO vê evento do qual não participa");
} else {
  fail("RLS eventos", `User 2 conseguiu ver evento! Status ${u2SeeEvent.status}: ${JSON.stringify(u2SeeEvent.data)}`);
}

// Add both users as participants
section("PASSO 1 — Tabela participantes + RLS sorteado_id");

// Use service role to add participants (since participantes has no INSERT policy yet)
const addP1 = await rest("POST", "/rest/v1/participantes",
  { evento_id: eventoId, usuario_id: user1Id, lista_desejos: "Quero um livro" },
...
if (addP1.status === 201 || addP1.status === 200) {
  ...
  fail("INSERT participantes p1", `Status ${addP1.status}: ${JSON.stringify(addP1.data)}`);
}

const addP2 = await rest("POST", "/rest/v1/participantes",
  { evento_id: eventoId, usuario_id: user2Id, lista_desejos: "Quero um jogo" },
  { Authorization: `Bearer ${SERVICE_KEY}`, Prefer: "return=minimal" }
);
if (addP2.status === 201 || addP2.status === 200) {
  ok("Participante 2 (User 2) adicionado via service_role");
} else {
  fail("INSERT participantes p2", `Status ${addP2.status}: ${JSON.stringify(addP2.data)}`);
}

// Add user3 as 3rd participant (needed for non-reciprocal derangement in perform-draw)
const addP3 = await rest("POST", "/rest/v1/participantes",
  { evento_id: eventoId, usuario_id: user3Id, lista_desejos: "Quero uma viagem" },
  { Authorization: `Bearer ${SERVICE_KEY}`, Prefer: "return=minimal" }
);
if (addP3.status === 201 || addP3.status === 200) {
  ok("Participante 3 (User 3) adicionado via service_role (para sorteio válido)");
} else {
  fail("INSERT participantes p3", `Status ${addP3.status}: ${JSON.stringify(addP3.data)}`);
}

// User 2 can now see the event (is a participant)
const u2SeeEventNow = await rest("GET", `/rest/v1/eventos?id=eq.${eventoId}&select=id,nome`,
  undefined, { Authorization: `Bearer ${user2Token}` }
);
if (u2SeeEventNow.status === 200 && Array.isArray(u2SeeEventNow.data) && (u2SeeEventNow.data as unknown[]).length === 1) {
  ok("RLS eventos: User 2 AGORA vê evento após ser adicionado como participante");
} else {
  fail("RLS eventos pós participação", `Status ${u2SeeEventNow.status}: ${JSON.stringify(u2SeeEventNow.data)}`);
}

// User 1 should ONLY see their own row in participantes (sorteado_id secret)
const u1SeePart = await rest("GET", `/rest/v1/participantes?evento_id=eq.${eventoId}&select=usuario_id,sorteado_id`,
  undefined, { Authorization: `Bearer ${user1Token}` }
);
if (u1SeePart.status === 200 && Array.isArray(u1SeePart.data)) {
  const rows = u1SeePart.data as { usuario_id: string; sorteado_id: string | null }[];
  if (rows.length === 1 && rows[0].usuario_id === user1Id) {
    ok("RLS participantes: User 1 vê APENAS seu próprio registro (sorteado_id secreto)");
  } else {
    fail("RLS participantes", `Retornou ${rows.length} rows. Esperava 1. Rows: ${JSON.stringify(rows)}`);
  }
} else {
  fail("SELECT participantes", `Status ${u1SeePart.status}: ${JSON.stringify(u1SeePart.data)}`);
}

// Test vw_participantes: User 1 can see all but only their sorteado_id
const u1SeeView = await rest("GET", `/rest/v1/vw_participantes?evento_id=eq.${eventoId}&select=usuario_id,sorteado_id`,
  undefined, { Authorization: `Bearer ${user1Token}` }
);
// Note: vw_participantes query bypasses row-level but applies CASE on column
...
if (u1SeeView.status === 200 && Array.isArray(u1SeeView.data)) {
  const rows = u1SeeView.data as { usuario_id: string; sorteado_id: string | null }[];
  const otherRowWithSecret = rows.find(r => r.usuario_id !== user1Id && r.sorteado_id !== null);
  if (otherRowWithSecret) {
    fail("VIEW vw_participantes segurança", `sorteado_id de outro usuário visível! ${JSON.stringify(otherRowWithSecret)}`);
  } else {
    ok(`VIEW vw_participantes: ${rows.length} rows visíveis, sorteado_id de outros é NULL`);
  }
} else {
  // If view is restricted due to RLS, that's also ok
  ok(`VIEW vw_participantes: Restrita por RLS (status ${u1SeeView.status})`);
}

// Test mensagens_privadas RLS
section("PASSO 1 — Tabela mensagens_privadas RLS");

// Insert a private message from user1 to user2 via service role (simulating post-draw)
const addMsg = await rest("POST", "/rest/v1/mensagens_privadas",
  { evento_id: eventoId, remetente_id: user1Id, destinatario_id: user2Id, texto: "Oi! Sou seu Amigo Secreto!" },
  { Authorization: `Bearer ${SERVICE_KEY}`, Prefer: "return=minimal" }
);
if (addMsg.status === 201 || addMsg.status === 200) {
  ok("Mensagem privada inserida (service_role)");
} else {
  fail("INSERT mensagens_privadas", `Status ${addMsg.status}: ${JSON.stringify(addMsg.data)}`);
}

// User 2 (destinatário) can see the message
const u2SeeMsg = await rest("GET", `/rest/v1/mensagens_privadas?evento_id=eq.${eventoId}&select=id,texto,remetente_id`,
  undefined, { Authorization: `Bearer ${user2Token}` }
);
if (u2SeeMsg.status === 200 && Array.isArray(u2SeeMsg.data) && (u2SeeMsg.data as unknown[]).length === 1) {
  ok("RLS mensagens_privadas: Destinatário (User 2) vê a mensagem");
} else {
  fail("RLS mensagens_privadas destinatário", `Status ${u2SeeMsg.status}: ${JSON.stringify(u2SeeMsg.data)}`);
}

// User4 is already created above (line ~98) and NOT part of this event (for RLS isolation test)
const u3SeeMsg = await rest("GET", `/rest/v1/mensagens_privadas?evento_id=eq.${eventoId}&select=id,texto`,
...
if (u3SeeMsg.status === 200 && Array.isArray(u3SeeMsg.data) && (u3SeeMsg.data as unknown[]).length === 0) {
  ok("RLS mensagens_privadas: Usuário externo (User 4) NÃO vê mensagens privadas de outros");
} else {
  fail("RLS mensagens_privadas terceiro", `User 4 viu mensagens! Status ${u3SeeMsg.status}: ${JSON.stringify(u3SeeMsg.data)}`);
}
...
// Test: Draw cannot run twice (status is 'sorteado')
...
// Verify sorteado_id was actually set (via service role, bypassing RLS)
const verifyDraw = await rest("GET", `/rest/v1/participantes?evento_id=eq.${eventoId}&select=usuario_id,sorteado_id`,
  undefined, { Authorization: `Bearer ${SERVICE_KEY}` }
);
if (verifyDraw.status === 200 && Array.isArray(verifyDraw.data)) {
  const rows = verifyDraw.data as { usuario_id: string; sorteado_id: string | null }[];
  const allAssigned = rows.every(r => r.sorteado_id !== null);
  const noSelfDraw = rows.every(r => r.usuario_id !== r.sorteado_id);
  
  if (allAssigned) ok("perform-draw: sorteado_id atribuído a todos os participantes");
  else fail("perform-draw sorteado_id", `Nem todos têm sorteado_id: ${JSON.stringify(rows)}`);
  ...
  const drawMap = new Map(rows.map(r => [r.usuario_id, r.sorteado_id]));
...
// Verify RLS: User 1 can only see their own sorteado_id after draw
const u1SeeSecret = await rest("GET", `/rest/v1/participantes?evento_id=eq.${eventoId}&select=usuario_id,sorteado_id`,
  undefined, { Authorization: `Bearer ${user1Token}` }
);
if (u1SeeSecret.status === 200 && Array.isArray(u1SeeSecret.data)) {
  const rows = u1SeeSecret.data as { usuario_id: string; sorteado_id: string | null }[];
  if (rows.length === 1 && rows[0].usuario_id === user1Id && rows[0].sorteado_id !== null) {
    ok("RLS pós-sorteio: User 1 vê APENAS seu próprio sorteado_id (preenchido)");
  } else {
...
const addMsg2 = await rest("POST", "/rest/v1/mensagens_privadas",
  { evento_id: eventoId, remetente_id: user2Id, destinatario_id: user1Id, texto: "Olá! Que bom que está participando!" },
  { Authorization: `Bearer ${SERVICE_KEY}`, Prefer: "return=minimal" }
);
...
const anonMsgRes = await fetch(`${SUPABASE_URL}/functions/v1/get-anonymous-messages?evento_id=${eventoId}`, {
  headers: {
    "Authorization": `Bearer ${user2Token}`,
    "apikey": ANON_KEY,
  },
});
const anonMsgData = await anonMsgRes.json() as { messages: { remetente_display: string; is_mine: boolean; texto: string }[] };
...
if (anonMsgRes.status === 200 && anonMsgData.messages) {
  ...
  // Only allowed UUIDs: evento_id and message id
...
const unauthRes = await fetch(`${SUPABASE_URL}/functions/v1/get-anonymous-messages?evento_id=${eventoId}`, {
