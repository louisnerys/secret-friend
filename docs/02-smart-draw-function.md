...
2. Implementar a lógica:
    - Autenticar o criador do event via JWT.
    - Buscar participants e exclusões usando `service_role` para bypassar RLS [7, 11].
    ...
    - Atualizar a tabela `participants` com os `drawn_id`.
    - Mudar status do event para 'drawn'.
