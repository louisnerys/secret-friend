...
export interface IEventRepository {
  ...
  createEvent(name: string, description: string, revealDate: string | null, creatorId: string): Promise<{ data: Event | null; error: any }>;
  getEventDetails(eventId: string): Promise<{ data: any; error: any }>;
  joinEvent(eventId: string, userId: string): Promise<{ error: any }>;
  updateWishlist(eventId: string, userId: string, wishlist: string): Promise<{ error: any }>;
  performDraw(eventId: string, accessToken: string): Promise<{ ok: boolean }>;
  addMessage(eventId: string, userId: string, text: string): Promise<{ error: any }>;
  ...
  getPrivateMessages(eventId: string, userId: string, drawnId: string | null): Promise<{ data: any; error: any }>;
  sendAnonymousMessage(eventId: string, text: string, toDrawer: boolean): Promise<{ error: any }>;
