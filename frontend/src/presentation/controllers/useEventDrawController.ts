...
export function useEventDrawController(eventId: string) {
  ...
  const fetchDrawnAndMessages = useCallback(async () => {
    ...
    const { data: privMsgs } = await eventDrawUseCase.getPrivateMessages(eventId, authData.user.id, me.drawn_id);

    if (privMsgs) {
      const formatted = privMsgs.map((m: PrivateMessage) => ({
        ...m,
        is_mine: m.sender_id === authData.user.id,
        sender_display: m.sender_id === authData.user.id ? 'You' : m.sender_display
      }));
  ...
  useEffect(() => {
    ...
    const setupRealtime = async () => {
      ...
      channel = supabase
        ...
        .on(
          'postgres_changes',
          { event: 'INSERT', schema: 'public', table: 'private_messages', filter: `event_id=eq.${eventId}` },
          (payload) => {
            const newMsg = payload.new as PrivateMessage;
            if (newMsg.sender_id !== myUserId && newMsg.receiver_id === myUserId) {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    const { error } = await eventDrawUseCase.sendAnonymousMessage(eventId, text, activeChat === 'drawer');
