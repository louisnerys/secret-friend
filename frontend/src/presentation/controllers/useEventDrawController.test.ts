...
import { mockGetUser, mockRpc, mockGetSession } from "../../../vitest.setup";
...
function createMockChain(data: any, isError = false) {
  const c: any = {
    ...
    order: vi.fn(() => c),
    single: vi.fn(() =>
      Promise.resolve({ data, error: isError ? { message: "error" } : null }),
    ),
    insert: vi
      .fn()
      .mockResolvedValue({ error: isError ? { message: "error" } : null }),
    then: vi.fn((resolve) =>
      resolve({ data, error: isError ? { message: "error" } : null }),
    ),
  };
  return c;
}
