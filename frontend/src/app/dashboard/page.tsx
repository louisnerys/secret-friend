    ...
    <div className="min-h-screen bg-surface font-body text-on-surface pb-32">
      ...
      <header className="bg-surface/80 backdrop-blur-xl fixed top-0 w-full z-50 shadow-sm shadow-black/5">
        <div className="flex items-center justify-between px-6 py-4 max-w-screen-xl mx-auto">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-primary-container flex items-center justify-center text-on-primary-container font-display font-bold text-sm" aria-hidden="true">
              {firstName[0]}
            </div>
      ...
      <main className="mt-24 px-6 max-w-screen-xl mx-auto space-y-10">
        ...
        <section className="relative pt-4 overflow-hidden">
          <div className="flex flex-col gap-2">
            ...
            <h2 className="text-4xl font-display font-bold text-on-surface leading-tight tracking-tight">
              ...
              <span className="text-primary">{t('dashboard.greeting_suffix')}</span>
            </h2>
          </div>
          <div className="absolute -top-10 -right-10 w-32 h-32 bg-secondary-container/20 rounded-full blur-3xl" aria-hidden="true" />
        </section>
        ...
        <section className="space-y-6">
          <div className="flex items-center justify-between">
            <h3 className="text-xl font-display font-bold text-on-surface">{t('dashboard.my_events')}</h3>
          </div>
          ...
          {events.length === 0 ? (
            <div className="bg-surface-container-low rounded-2xl p-10 text-center space-y-4">
              ...
              <h4 className="font-display text-xl font-bold text-on-surface">{t('dashboard.no_events_title')}</h4>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {events.map((event) => (
                <div
                  ...
                  role="button"
                  tabIndex={0}
                  onKeyDown={(e) => e.key === 'Enter' && router.push(`/evento/${event.id}`)}
                >
        ...
        <section className="pb-4">
          <h3 className="text-xl font-display font-bold text-on-surface mb-6">{t('dashboard.gift_inspiration')}</h3>
          <div className="flex gap-4 overflow-x-auto pb-4 -mx-6 px-6" style={{ scrollbarWidth: 'none' }}>
            {[
              { label: t('dashboard.guides.label_2024'), name: t('dashboard.guides.wine'), emoji: '🍷' },
              { label: t('dashboard.guides.label_minimalist'), name: t('dashboard.guides.stationery'), emoji: '📓' },
              { label: t('dashboard.guides.label_executive'), name: t('dashboard.guides.tech'), emoji: '⌚' },
              { label: t('dashboard.guides.label_gourmet'), name: t('dashboard.guides.gourmet'), emoji: '🧺' },
            ].map((item) => (
              <div
                key={item.name}
                ...
                tabIndex={0}
              >
                <div className="w-full h-32 bg-gradient-to-br from-secondary-container/40 to-primary-container/20 flex items-center justify-center text-5xl" aria-hidden="true">
                  {item.emoji}
                </div>
                <div className="p-3">
      ...
      <nav className="fixed bottom-0 left-0 w-full flex justify-around items-center px-4 pb-6 pt-3 bg-surface/90 backdrop-blur-2xl z-50 rounded-t-3xl shadow-[0_-8px_24px_rgba(26,28,26,0.06)]">
        <div className="flex flex-col items-center justify-center bg-primary-container text-on-primary-container rounded-full px-5 py-2 transition-all duration-300">
          ...
          <span className="font-label text-[10px] uppercase tracking-widest font-semibold mt-1">{t('dashboard.nav_events')}</span>
        </div>
        <div
          onClick={() => router.push('/admin')}
          ...
          tabIndex={0}
        >
          <span className="material-symbols-outlined" style={{ fontSize: 22 }} aria-hidden="true">auto_awesome</span>
          <span className="font-label text-[10px] uppercase tracking-widest font-semibold mt-1">{t('common.admin')}</span>
        </div>
        <div
          onClick={handleLogout}
          className="flex flex-col items-center justify-center text-on-surface-variant/50 p-2 hover:text-primary transition-colors cursor-pointer"
        ...
        >
          ...
          <span className="font-label text-[10px] uppercase tracking-widest font-semibold mt-1">{t('dashboard.nav_profile')}</span>
        </div>
      </nav>
    </div>
  );
