...
describe("Dashboard", () => {
    ...
    (supabase.from as any).mockImplementation((table: string) => {
      if (table === "users")
        return {
          ...
          single: vi
            .fn()
            .mockResolvedValue({ data: { name: "A" }, error: null }),
        };
      if (table === "events")
        return {
          select: vi.fn().mockResolvedValue({
  ...
  it("renders admin button if user is admin", async () => {
    ...
    await waitFor(() => {
      expect(
        container.querySelector("button.uppercase.text-secondary"),
      ).not.toBeNull();
    });
    const adminBtn = container.querySelector(
      "button.uppercase.text-secondary",
    ) as HTMLElement;
