...
export default function Login() {
  ...
  const handleSocialLogin = async (provider: 'google' | 'github') => {
    ...
    // Note: If successful, it will redirect, so no need to stop loading state
  };
        ...
        <div className="w-full max-w-md py-24 lg:py-0">
          <header className="mb-10 text-center">
            ...
            <p className="text-on-surface-variant font-medium">
              Faça login com sua conta social para continuar.
            </p>
          </header>
          ...
          <div className="space-y-4">
            {/* Error */}
            {error && (
              <div className="p-3 text-sm text-on-error bg-error rounded-lg text-center">
                {error}
              </div>
            )}
            ...
            <button
              ...
              className="flex items-center justify-center gap-3 w-full py-4 bg-white text-gray-800 rounded-full border border-gray-300 font-bold tracking-tight hover:bg-gray-50 transition-all duration-300 active:scale-[0.98] disabled:opacity-70 disabled:active:scale-100"
            >
              {loadingGoogle ? (
                'Conectando...'
              ) : (
            ...
            <button
              ...
              disabled={loadingGoogle || loadingGithub}
              className="flex items-center justify-center gap-3 w-full py-4 bg-[#24292F] text-white rounded-full font-bold tracking-tight hover:bg-[#24292F]/90 transition-all duration-300 active:scale-[0.98] disabled:opacity-70 disabled:active:scale-100"
            >
              {loadingGithub ? (
                'Conectando...'
              ) : (
